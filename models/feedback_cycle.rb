class FeedbackCycle < ActiveRecord::Base
  ...
  def start(execute=false)
    
    kudozio = User.where( "is_kudozio = ?", true ).first
    
    if !self.team.nil? && !kudozio.nil?
  ...
  def finish(execute=false)
    ...
    if !kudozio.nil? && !self.team.nil?
      
      kudozio_account = kudozio.accounts.first
    
      self.team.transactions.select { |t| t.created_at >= start_time && !t.origin.user.is_kudozio }.each do |tx|
      ...
      self.team.users.each do |usr|
        ...
        if remove > 0
          ...
          tx = Transfer.new
          tx.ammount = remove*-1
          tx.message = "Give these Kudoz back to me! Next time remember to give away 100 kudoz. :P"
          tx.origin = kudozio_account
          tx.destination = source_account
